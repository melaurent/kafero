...
func GetFs(t *testing.T) kafero.Fs {
	return __getS3Fs(t)
}
...
func testWriteFile(t *testing.T, fs kafero.Fs, name string, size int) {
...
func testCreateFile(t *testing.T, fs kafero.Fs, name string, content string) {
...
func TestRename(t *testing.T) {
	...
	// Renaming of a directory isn't tested because it's not supported by kafero in the first place
}
...
func TestFileReaddir(t *testing.T) {
	...
	reader1 := NewLimitedReader(rand.New(rand.NewSource(0)), 1000)

	file, errOpen := fs.OpenFile("/dir1/readme.txt", os.O_WRONLY, 0777)
	if errOpen != nil {
		t.Fatal("Could not open file:", errOpen)
	}
